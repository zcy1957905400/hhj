...
public class RegionServiceImpl implements RegionService {
    ...
    protected static Logger log = LoggerFactory.getLogger(RegionServiceImpl.class);
...
//    }
    ...
    public ResultVO<List<AllDTO>> findAllRegion() {
        List<Region> regionList=regionRepository.findAllRegion();
        List<AllDTO> allDTOList=new ArrayList<>();
        for (Region region:regionList) {
            AllDTO allDTO=new AllDTO();
            allDTO.setId(region.getId());
            allDTO.setName(region.getName());
            allDTOList.add(allDTO);
        }
        return new ResultVO<>(ResultCode.SUCCESS,allDTOList);
    }
